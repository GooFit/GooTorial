  ...
  , calculators(0) 
{
  ...
  cudaMemcpyFromSymbol((void**) &host_fcn_ptr, ptr_to_incoherent, sizeof(void*));
  initialise(pindices); 
...
__host__ void IncoherentSumThrustFunctor::setDataSize (unsigned int dataSize, unsigned int evtSize) {
...
__host__ fptype IncoherentSumThrustFunctor::normalise () const {
